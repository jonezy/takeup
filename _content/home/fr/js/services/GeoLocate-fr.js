     ...
     var init = function(){ loadLocalDB(); getGeoLocation(); };
	  ...
	  var getGeoLocation = function (){
		 ...
		 $("#form-modal").fadeOut();
		  ...
		  $("#form-modal").fadeOut();
		  bannersToPlay = [];
		  bannersToPlay = ['#geo-banner','#slide-2','#slide-3','#slide-4'];
		...
		if (Modernizr.geolocation) {
		  ...
		  var img = '<a href="http://www.mini.ca/fr/select_dealer/index.jsp?bcId=/fr/index.html" target="_parent"><img src="images/find-retailer-img.jpg" alt="Locate A Dealer Near You!" border="0" /></a>';
	  ...
	  var getListOfRetailersPerProvince = function( p, postal){
		  ...
		  dcsMultiTrack( 'DCS.dcsuri', ' /Geo_Target_/'+p+'/','WT.cg_n', 'PS:New Cars:Model:R61', 'WT.dl', '0');
		  ...
		  for( var i = 0; i < retailers.length; i++){
			  ...
			  //console.log( retailers[i].Name + " :: " + retailers[i].City + " :: " + retailers[i].Province );
		...
		if( c == 0 ){
		  $("#locate-dealers").html('');
		  var img = '<a href="http://www.mini.ca/fr/select_dealer/index.jsp?bcId=/en/index.html" target="_parent"><img src="images/find-retailer-img.jpg" alt="Locate A Dealer Near You!" border="0" /></a>';
		  $("#locate-dealers").html(img);
	  ...
	  var displayDealerLocations = function () {
		  ...
		  dealerLats.sort(function(a,b) { return a.Distance - b.Distance; });
		   /*dealerLats.sort(function(m1,p1) { 
		  ...
		  for(var i = 0; i < dealerLats.length; i++){
		   ...
		   dealer+= '<span class="d-title"> <a href="mailto:'+email+'?subject=Contact from MINI.ca" target="_blank"> > Contactez détaillant</a></span><br>';
		   dealer+= '<span class="d-title"> <a href="'+website+'" target="_parent"> > Visiter Site</a></span>';
		   dealer+='</div>';
		  ...
		  userDisplay = 'Vous êtes actuellement en '+ dealerLats[0].Province +' <span class="change-location"><br><a href="#">( changer )</a> </span>';
		  $("#user-provice").css({'top' : '575px' });
		  }else{
		  userDisplay = 'Vous êtes actuellement en '+ userProvince +' <span class="change-location"><br><a href="#">( changer )</a> </span>';
		  $("#user-provice").css({'top' : '575px' });
		  }
	  ...
	  var regions = [{reg : 'Central', province : 'Ontario'},{reg : 'East', province : 'New Brunswick'},{reg : 'East', province : 'Newfoundland and Labrador'},{reg : 'East', province : 'Newfoundland'},{reg : 'East', province : 'Nova Scotia'},{reg : 'East', province : 'Prince Edward Island'},{reg : 'East', province : 'Quebec'},{reg : 'West', province : 'Alberta'},{reg : 'West', province : 'British Columbia'},{reg : 'West', province : 'Manitoba'},{reg : 'West', province : 'Northwest Territories'},{reg : 'West', province : 'Nunavut'},{reg : 'West', province : 'Saskatchewan'},{reg : 'West', province : 'Yukon'}];
		...
		if(r == 'East'){ turnOffAllBanners(); bannersToPlay = ['#geo-banner','#slide-2','#slide-3','#slide-4']; _banner.Rotator.init( region, bannersToPlay ); return;};
		if(r == 'West'){ turnOffAllBanners(); bannersToPlay = ['#geo-banner','#slide-2','#slide-3','#slide-4']; _banner.Rotator.init( region, bannersToPlay ); return;};
		if(r == 'Central'){ turnOffAllBanners(); bannersToPlay = ['#geo-banner','#slide-2','#slide-3','#slide-4']; _banner.Rotator.init( region, bannersToPlay ); return;};
	
		/*if(r == 'East'){ turnOffAllBanners(); bannersToPlay = ['#east-banner','#slide-2','#slide-3','#slide-4']; _banner.Rotator.init( region, bannersToPlay ); return;};
		if(r == 'West'){ turnOffAllBanners(); bannersToPlay = ['#central-banner','#slide-2','#slide-3','#slide-4']; _banner.Rotator.init( region, bannersToPlay ); return;};
		if(r == 'Central'){ turnOffAllBanners(); bannersToPlay = ['#central-banner','#slide-2','#slide-3','#slide-4']; _banner.Rotator.init( region, bannersToPlay ); return;};*/
			  }else{
				  bannersToPlay = ['#geo-banner','#slide-2','#slide-3','#slide-4'];
			  }
